...
- VPS where you can host NexTerm, you can use one from [Hetzner](https://go.bitdoze.com/hetzner) You can use a VPS to have Memos installed but performances will not be that good. In our test we are using a 8 CPUs 16 GB RAM and is bearly moving.
...
services:
  nexterm:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.memos.rule=Host(`nexterm.domain.com`)
      - traefik.http.routers.memos.entrypoints=https
      - traefik.http.services.memos.loadbalancer.server.port=6989
