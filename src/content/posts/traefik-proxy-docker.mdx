...
services:
  traefik:
    ...
    command:
      ...
      - --certificatesresolvers.letsencrypt.acme.email=bitdoze1@gmail.com
...
1. `--api.insecure=true`
   ...
   - This enables Traefik's API and dashboard without security restrictions. It's not recommended for production use.
...
7. `traefik.http.middlewares.traefik-auth.basicauth.users=${TRAEFIK_DASHBOARD_CREDENTIALS}`
   ...
   - Sets up HTTP Basic Authentication for the "traefik-auth" middleware.
