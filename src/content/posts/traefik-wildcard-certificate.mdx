...
- Defines two secrets: `cloudflare-token` and `cloudflare-email`, which are stored in separate files.
...
- `traefik.http.middlewares.traefik-auth.basicauth.users=${TRAEFIK_DASHBOARD_CREDENTIALS}`: Sets up basic auth for the dashboard.
