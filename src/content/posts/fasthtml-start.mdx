...
FastHTML seamlessly integrates with [HTMX](https://htmx.org/), a library that allows you to access AJAX, CSS Transitions, WebSockets and Server Sent Events directly in HTML, without writing JavaScript. FastHTML includes HTMX by default, but we're including it explicitly in this example for clarity.
...
def home():
    return Html(
        Head(
            Title("HTMX Counter Example"),
            # Include HTMX library - FastHTML actually includes this by default
        ...
        Body(
            Div(
                ...
                Div(
                    ...
                    Button("Increment",
                           # Make a POST request to /increment when clicked
                           hx_post="/increment",
                           # Update the element with id="counter"
                           hx_target="#counter",
                           cls="bg-blue-500 text-white px-4 py-2 rounded mr-2"),

                    # Decrement button with HTMX attributes
                    Button("Decrement",
                           # Make a POST request to /decrement when clicked
                           hx_post="/decrement",
                           # Update the element with id="counter"
                           hx_target="#counter",
